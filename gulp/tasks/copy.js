...
// copy feed to dist folder
gulp.task('feeds', 'Copy feeds to `dist/`', function () {
  return gulp.src(config.copyFeeds.src)
    .pipe(gulp.dest(config.copyFeeds.dest));
});
...
gulp.task('copy', 'Copy fonts and extras to `dist/` folder', ['feeds', 'fonts', 'extras', 'icons']);